...
class Trainer(BaseTrainer):
    def __init__(self,param_vals,horizon_recursive, **args):
        self.horizon_recursive=horizon_recursive
    ...
    def _calculate_supports(self, adj_mat, filter_type):
        ...
        if torch.cuda.is_available(): ## added K, to run in cpu
            supports = [torch.tensor(i).cuda() for i in supports]  ## K check: for cuda
        return supports
    ...
    def train(self):
        ...
        for epoch in range(self._max_epochs):
            ...
            for i, (X, label) in enumerate(self.data['train_loader']):
                X, label = self._check_device([X, label]) #torch.Size([64, 16, 40, 17]), torch.Size([64, 8, 40, 1]))
                output_tensor=X
                for t in range(args.horizon_recursive): ## which is 8
                ## torch.randn
                    t_minus_8 = output_tensor[:, -8, :, :].unsqueeze(1)
                    trimmed_tensor = output_tensor[:, 1:, :, :]
                    output_tensor = torch.cat([trimmed_tensor, t_minus_8], dim=1)
                    # print('4',output_tensor)
